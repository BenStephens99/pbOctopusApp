...
export default function PropertyDisplay(props) {
  ...
  const gasCost = calculateGas(props.usage.gas, account.gasStandingCharges, account.gasUnitRates);

  if (typeof sessionStorage !== 'undefined') {
    sessionStorage.setItem(
      `${account.id}-${property.postcode.replace(/\s/g, '')}`,
  ...
  return (
    <a
      href={`/properties/${account.id}?postcode=${property.postcode.replace(/\s/g, '')}`}
      className="w-full duration-25 hover:brightness-90"
    >
