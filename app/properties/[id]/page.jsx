...
import { Card, CardHeader, CardBody, Tabs, Tab, Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Divider } from "@nextui-org/react";
import Loading from "@/app/components/loading";
import UsageChart from "./Components/UsageChart";
import { getElectricUsage, getElectricUnitRates, getElectricStandingCharges, getGasUsage, getGasUnitRates, getGasStandingCharges } from "@/app/actions/octopus";
...
export default function Page({ params }) {
    ...
    const views = {
        ...
        past12Months: {
            name: 'Past 12 Months',
            period: 'month',
            startDate: new Date(new Date(new Date().setFullYear(new Date().getFullYear() - 1)).setDate(1)).toISOString(),
            endDate: new Date().toISOString(),
        },
    }
    ...
    useEffect(() => {
        ...
        setElectricCost(calculateElectric(
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            const electricUsage = await getElectricUsage(account.api_key, property.mpan, property.electricSerialNumbers, views[view].startDate, views[view].endDate, views[view].period);
            ...
            const gasUsage = await getGasUsage(account.api_key, property.mprn, property.gasSerialNumbers, views[view].startDate, views[view].endDate, views[view].period);
        ...
        <div>
            <div className="flex flex-col gap-3">
                <div className="flex flex-col">
                    <h1 className="capitalize">{property.addressLine1 ? property.addressLine1?.toLowerCase() :'...'}</h1>
